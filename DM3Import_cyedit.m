...
%
%Edited by Alex Powers 15 Jan 2015, now reads display parameters from dm3
%file: the max and min brightness. It was found without these parameters,
%overly saturated pixels would dominate the image when displaying. With a
%maximum brightness, all pixels with intensity above that intensity are
%lowered to the maximum for display giving an image that actually looks
%like that produced by the microscope. 
%
...
function [dm3struct] = DM3Import_cyedit( dm3filename )
    global dm3file;
    global endianstyle;


    % Search for .dm3 file extension.  If not present, append it.
    % in the file name btw...if file name doesn't end in .dm3 it's added 
    ...
    %frewind resets position to beginning so the whole thing is read out
    %as...a char??
    frewind(dm3file);
    metadm3file = fread(dm3file, 'uchar=>char' )';
    %I guess N would be size
    N = numel(metadm3file);
    ...
    if( spectracount == 0 ) % IMAGES
            ...
            temploc = regexp(metadm3file, 'HighLimit%%%%');
            highlimloc = []; 
            % CHELSEA EDIT: an image with only 1 read out temploc? will
            % result in highlimloc = [] and failure in the next line (303)
%             for index = 1:numel(temploc)
%                 if temploc(index) > calibrateloc_curr
%                     highlimloc = [highlimloc, temploc(index)];
%                 end
%             end
%            
%             if numel(temploc) == 1 
...
%             
%             if numel(temploc) == 0 
%                 %I'm putting in a random value because the tifs aren't
%                 %working - CHelsea 
%                 highlimloc = 1000; 
%             end 
%             %do I even USE the high limit? probably during the processing?
%             %But what to do if there's no high limit???? 
%             dm3struct.intensity = struct( 'highlimit', readTagData( highlimloc(1) ), ...
...
% 
        ...
        for spectraindex = 1:spectracount
            ...
            highlimloc  = regexp(metadm3file, 'HighLimit%%%%');
            highlim = readTagData( highlimloc); 
            lowlimloc = regexp(metadm3file, 'LowLimit%%%%');
            lowlim = readTagData( lowlimloc); 
            dm3struct.intensity = struct('highlim', highlim, 'lowlim', lowlim,...
                     'scale', readTagData( scaleloc(1) ), ...
                    'origin', readTagData( originloc(1) ), ...
                    'units', char( readTagData( unitloc(1) ).' ) );    
...
% scales.
    ...
    ecurrentloc = regexp( metadm3file, 'Emission Current \(A\)%%%%');
